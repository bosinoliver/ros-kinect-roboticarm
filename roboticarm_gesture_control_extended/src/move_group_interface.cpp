...
int main(int argc, char** argv) {
	...
	move_group_arm.setPlanningTime(0.04);
	...
	move_group_gripper.setPlanningTime(0.04);
	...
	while (node_handle.ok()) {
		...
		try {
			...
			transformStamped_shoulder_to_elbow = tfBuffer.lookupTransform(
					"left_shoulder_1", "left_elbow_1", ros::Time(0));

			transformStamped_elbow_to_hand = tfBuffer.lookupTransform(
					"left_elbow_1", "left_hand_1", ros::Time(0));
	...
	shoulder_to_elbow_vector[0] =
			transformStamped_shoulder_to_elbow.transform.translation.x;
	shoulder_to_elbow_vector[1] =
			transformStamped_shoulder_to_elbow.transform.translation.y;
	shoulder_to_elbow_vector[2] =
			transformStamped_shoulder_to_elbow.transform.translation.z;

	elbow_to_hand_vector[0] =
			transformStamped_elbow_to_hand.transform.translation.x;
	elbow_to_hand_vector[1] =
			transformStamped_elbow_to_hand.transform.translation.y;
	elbow_to_hand_vector[2] =
			transformStamped_elbow_to_hand.transform.translation.z;

	/*calculate armlength of user*/

	double armlength = shoulder_to_elbow_vector.length()
			+ elbow_to_hand_vector.length();

	/*calculate conversion factor for position target*/

	double conversion_factor = 0.43 / armlength;
	...
	while (node_handle.ok()) {
		...
		try {
			...
			transformStamped_left_hand_1 = tfBuffer.lookupTransform("torso_1",
					"left_hand_1", ros::Time(0));
		...
		distance[0] = conversion_factor
				* (transformStamped_left_hand_1.transform.translation.x - 0.15);
		distance[1] =
				conversion_factor
						* (transformStamped_left_hand_1.transform.translation.z
								* (-1.0));
		distance[2] = conversion_factor
				* (transformStamped_left_hand_1.transform.translation.y - 0.15);

		distance = distance.rotate(rotate, 0.600);

		if(distance.length() >= 0.455){
					distance *= (0.455 / distance.length());
				}

		distance[0] = round(1000.0 * distance[0]) / 1000.0;
		distance[1] = round(1000.0 * distance[1]) / 1000.0;
		distance[2] = round(1000.0 * distance[2]) / 1000.0;


		move_group_arm.setPositionTarget(distance[0], distance[1], distance[2]);

		ROS_INFO_NAMED("planning", "x : %lf y: %lf z: %lf", distance[0],
								distance[1], distance[2]);
		ROS_INFO_NAMED("planning", "distance : %lf", distance.length());
		bool succes = (move_group_arm.plan(my_plan)
		...
		if (succes) {
			...
			succes = (move_group_arm.plan(my_plan)
					== moveit::planning_interface::MoveItErrorCode::SUCCESS);
			...
			if (succes) {
				...
				joint_group_positions = move_group_gripper.getCurrentJointValues();
						ROS_INFO_NAMED("Gripper Values", "Gripper: %lf %lf %lf", joint_group_positions[0], joint_group_positions[1], joint_group_positions[2]);

						if ((transformStamped_right_hand_1.transform.translation.y - 0.15) > 0.0) {

							joint_group_positions[0] = 0.630;
						...
						if((move_group_gripper.plan(my_plan)
								== moveit::planning_interface::MoveItErrorCode::SUCCESS)){
