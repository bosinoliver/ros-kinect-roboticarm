...
\section{Bilder}
\index{Bilder}
Wenn die Bilder nicht selber erstellt wurden, dann müssen sie auf alle Fälle zitiert werden. Schwierig hierbei ist das Copyright. Um hier Problemen aus dem Weg zu gehen, wird empfohlen, keine Bilder aus dem Internet zu verwenden. Besser und unproblematischer ist es, selber einfache Zeichnungen zu erstellen. Bilder die in der WE4 erstellt wurden, dürfen verwendet werden, müssen aber mit Zitat (die Intranet Seite) gekennzeichnet werden.
...
Als Beispiel zeigt die Abbildung \ref{fig:rumbler} das Kettenfahrzeug FTW-TV5 Rumbler\index{Rumbler} welches in der Wissenschaftlichen Einrichtung 4 für autonome Fahrversuche verwendet wird.

\begin{figure}
	\centering
		\includegraphics[width=0.6\textwidth]{images/results/rumbler.png}
	\caption{Autonomes Kettenfahrzeug der Wissenschaftlichen Einrichtung 4}
	\label{fig:rumbler}
\end{figure}
...
\lstinputlisting[language=vhdl,basicstyle=\small,
frame=none,label=SN74x6541,
caption=Sourcecode SN74x6541]{source/SN74x6541.vhd}


\textbf{Nicht} ausreichend ist es, nur den Quelltext in der Arbeit aufzunehmen. Die Funktion des Programmes muss erläutert werden. So kann beispielsweise die Aufteilung in mehrere Tasks über ein Sequenzendiagramm dargestellt werden. Meist sind Grafiken und Diagramme besser als reiner Text. Selbstverständlich müssen auch die Grafiken bzw. Digramme erklärt werden.
